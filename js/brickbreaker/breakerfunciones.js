  ...
  for (i = 0; i < ladrilloColumna; i++) {
    for (l = 0; l < ladrilloFila; l++) {
      if (ladrillos[i][l].status == 1) {
        let ladrilloX =
          i * (ladrilloAncho + ladrilloPadding) + ladrilloMarginLeft;
        let ladrilloY =
          l * (ladrilloAlto + ladrilloPadding) + ladrilloMarginTop;
...
function dibujar() {
  ctx.clearRect(
    ...
    canvas.width,
    canvas.height
  ); /*Esto permite borrar el fotograma anterior que se dibujo, para no dejar rastro de la bola*/
  dibujarPuntaje();
  colision();
  dibujarPaleta();
  dibujarLadrillos();
